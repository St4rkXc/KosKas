...
<template>
  ...
  <Transition name="slide-up">
    <div
      v-if="isOpen"
      class="fixed bottom-0 left-0 right-0 bg-[#121212] z-50 flex flex-col"
      style="padding-bottom: env(safe-area-inset-bottom)"
    >
      <!-- Display -->
      <div class="p-6 flex flex-col items-end border-b border-white/5">
        <div class="text-[#71717A] text-sm mb-1 uppercase tracking-wider font-sans">
          Pengeluaran
        </div>
      ...
      <div class="flex overflow-x-auto gap-2 p-4 no-scrollbar border-b border-white/5 bg-[#0A0A0A]">
        <button
          ...
          :class="['px-4 py-2 rounded font-sans text-sm whitespace-nowrap transition-colors border',
            selectedPocketId === pocket.id ? `${pocket.colorClass} border-transparent` : 'bg-[#1E1E1E] text-[#71717A] border-[#1E1E1E]'
          ]"
        >
          {{ pocket.name }}
        </button>
      ...
      <div class="grid grid-cols-3 gap-1 p-1 bg-black">
        <button
          ...
          class="bg-[#121212] h-16 sm:h-20 flex items-center justify-center text-2xl font-mono font-bold active:bg-white active:text-black transition-colors text-white"
        >
          <template v-if="key === 'DEL'">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 5H9l-7 7 7 7h11a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2Z"/><line x1="18" y1="9" x2="12" y2="15"/><line x1="12" y1="9" x2="18" y2="15"/></svg>
          </template>
          <template v-else>
            {{ key }}
          </template>
      ...
      <button
        ...
        :class="['h-16 w-full font-sans font-bold text-lg uppercase tracking-wider transition-colors',
          (amount > 0 && selectedPocketId) ? 'bg-[#10B981] text-black active:bg-[#059669]' : 'bg-[#1E1E1E] text-[#71717A] cursor-not-allowed'
        ]"
      >
        Simpan
      </button>
