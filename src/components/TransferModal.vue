...
const isTransferValid = computed(() => {
  return (
    fromPocketId.value &&
    toPocketId.value &&
    fromPocketId.value !== toPocketId.value &&
    amount.value > 0 &&
    amount.value <= fromPocketBalance.value
  );
});
    ...
    >
      ...
      <div class="p-6 flex justify-between items-center border-b border-white/5">
        <h2 class="text-white font-sans font-bold uppercase tracking-wider text-sm flex items-center gap-2">
          <ArrowLeftRight :size="16" class="text-[#10B981]" />
      ...
      <div class="p-6 flex flex-col items-end border-b border-white/5 bg-[#0A0A0A]">
        <div class="text-[#71717A] text-xs mb-1 uppercase tracking-wider font-sans">
          Jumlah Transfer
        </div>
        ...
        <div class="text-[10px] font-mono mt-1" :class="amount > fromPocketBalance ? 'text-[#EF4444]' : 'text-[#71717A]'">
          Saldo Pengirim: {{ formatRupiah(fromPocketBalance) }}
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-5 no-scrollbar">
        ...
        <div>
          <label class="text-[#71717A] text-[10px] uppercase font-bold block mb-2">Dari Pocket (Sumber)</label>
          <div class="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
            <button
              v-for="pocket in store.pockets"
              :key="`from-${pocket.id}`"
              @click="selectFromPocket(pocket.id)"
              :class="['px-3.5 py-2.5 rounded font-sans text-xs whitespace-nowrap transition-colors flex items-center gap-2 border',
                fromPocketId === pocket.id ? `${pocket.colorClass} border-transparent` : 'bg-[#1E1E1E] text-[#71717A] border-[#1E1E1E]'
              ]"
            >
        ...
        <!-- To Pocket Selector -->
        <div>
          <label class="text-[#71717A] text-[10px] uppercase font-bold block mb-2">Ke Pocket (Tujuan)</label>
          <div class="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
            <button
              ...
              :class="['px-3.5 py-2.5 rounded font-sans text-xs whitespace-nowrap transition-colors flex items-center gap-2 border',
                toPocketId === pocket.id ? `${pocket.colorClass} border-transparent` : 'bg-[#1E1E1E] text-[#71717A] border-[#1E1E1E]'
              ]"
            >
              <component :is="iconMap[pocket.icon]" :size="12" />
              <span>{{ pocket.name }} ({{ formatRupiah(store.pocketBalances[pocket.id] || 0) }})</span>
        ...
        <!-- Optional Note -->
        <div>
          <label class="text-[#71717A] text-[10px] uppercase font-bold block mb-2">Catatan (Optional)</label>
          <input 
            v-model="transferNote"
            type="text"
            placeholder="e.g. Alokasi lebih pangan, Tambahan kos"
            class="w-full bg-[#1E1E1E] border border-white/5 px-3 py-2 text-white text-sm rounded focus:outline-none focus:border-[#10B981]"
          />
        </div>
      </div>

      <!-- Custom Keypad -->
      <div class="grid grid-cols-3 gap-1 p-1 bg-black flex-shrink-0">
        <button
      ...
      <button
        ...
        :class="['h-16 w-full font-sans font-bold text-base uppercase tracking-wider transition-colors',
          isTransferValid ? 'bg-[#10B981] text-black active:bg-[#059669]' : 'bg-[#1E1E1E] text-[#71717A] cursor-not-allowed'
        ]"
      >
        Lakukan Transfer
      </button>
